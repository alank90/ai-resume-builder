...
def get():
    ...
    with st.expander("Fill out your profile"):
        ...
        name = st.text_input("Enter your name: ",
                             placeholder="Full name", key=counter)
        counter += 1
        name = "Name: \n" + name
        current_position = st.text_input(
            "Enter your current position: ", key=counter)
        counter += 1

        current_position = "Current Title: \n" + current_position
        email = st.text_input("Ënter your email: ", key=counter)
        counter += 1

        email = "Email: \n" + email
        mobile = st.text_input("Enter your mobile number: ",
                               placeholder="i.e. 555 233-1212", key=counter)
        counter += 1

        mobile = "Mobile: \n" + mobile

        summary = st.text_area(
            "Please give a brief summary of yourself", key=counter)
...
# ------------------------------------------------------------------------------------------------------------ #
    with st.expander("Enter your Work Experience details: "):
        ...
        while True:
            ...
            resume_builder.Experience.job_role = st.text_input(
                "Enter your job role: ", key=counter)
            counter += 1

            if resume_builder.Experience.job_role.lower() == 'done':
                break

            resume_builder.Experience.company_name = st.text_input(
                "Enter the company name: ", key=counter)
            counter += 1

            resume_builder.Experience.job_description = st.text_input(
                f"Enter the description for '{resume_builder.Experience.job_role}': ", key=counter)
            counter += 1

            # Prompt user to add Experience details
            exp_input = st.text_input(
                "Do you want to add another experience detail? (yes/no)", key=counter).lower()
            if exp_input == "yes":
                # Append answers to experience list
                experience.append(
                    {"job_role": resume_builder.Experience.job_role, "company_name": resume_builder.Experience.company_name,
                     "description": resume_builder.Experience.job_description})
                counter += 1
            else:
                break
...
# ----------------------------------------------------------------------------------------------------------- #
    ...
    return resume_builder.Resume(name, email, current_position, mobile, summary), resume_builder.Experience, resume_builder.Education
